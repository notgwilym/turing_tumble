<script lang="ts">
    ...
    let configs = $state<PieceAnimConfig[]>(
        PIECE_ANIM_CONFIGS.map(c => JSON.parse(JSON.stringify(c)))
    );
    ...
    // Initialize from exported ANIM_PATHS (survives page reload if pasted into config)
    let savedPaths = $state<SavedPath[]>(
        (ANIM_PATHS ?? []).map((p: any) => ({
            key: p.key,
    ...
    // Speed state (shared with Tab 3, but declared here so savePath can access)
...
function handleCurvePointerMove(e: PointerEvent) {
    ...
    function handleExport() {
        const configsOut = configs.map(c => ({
            type: c.type, label: c.label, svgPath: c.svgPath,
            viewBoxWidth: c.viewBoxWidth, viewBoxHeight: c.viewBoxHeight,
            dimensions: c.dimensions, centre: c.centre,
            entryFromLeft: c.entryFromLeft, entryFromRight: c.entryFromRight,
            exitToLeft: c.exitToLeft, exitToRight: c.exitToRight,
            flippable: c.flippable, selfAnimates: c.selfAnimates,
        }));
        const pathsOut = savedPaths.map(p => ({
            key: p.key, mode: p.mode, pathD: p.d, events: p.events, speed: p.speed, duration: p.duration,
        }));
